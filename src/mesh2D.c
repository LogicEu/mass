...
Mesh2D mesh2D_create(void)
{
    ...
    mesh.vertices = vector_create(sizeof(vec2));
...
Mesh2D mesh2D_shape_polygon(const size_t sides)
{
    ...
    for (size_t i = 1; i < sides; ++i) {
        ...
        vector_push(&mesh.vertices, &center);
        vector_push(&mesh.vertices, &p1);
        vector_push(&mesh.vertices, &p2);
    ...
    vector_push(&mesh.vertices, &center);
    vector_push(&mesh.vertices, &p1);
    vector_push(&mesh.vertices, &orig);
