...
void vertex_array_push_quad_z(array_t* vertices, array_t* indices, float x, float y)
{
    ...
    for (unsigned int i = 0; i < 6; i++) {
        ...
        array_push(indices, &vertices->size);
...
{
    array_t normals = array_reserve(positions->bytes, positions->size);
    const unsigned int triangles = positions->size / 3;
...
{
    const unsigned int face_count = indices->size / 3;
